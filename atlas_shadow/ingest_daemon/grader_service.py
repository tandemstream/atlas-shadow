...
def _resolve_commit_for_ledger(repo_path: Path, commit_ref: str) -> str:
    """Resolve short receipt SHAs before ingest-ledger lookup."""
    ...
    if proc.returncode != 0:
        return ref
    return proc.stdout.strip() or ref
...
def run_pr_grading(
    ...
    *,
    ...
    ``revision_pin_mode`` stays ``"event-base"`` for the live PR gate.
    Offline batch grading may pass ``"receipt-source"`` so each code receipt
    queries Atlas at the code revision for its authored ``source_commit``.
    """
...
def _serialize_row(row: "pr_comment_mod.ReceiptGradingRow") -> dict[str, Any]:
    ...
    return {
        ...
        # Receipt-source baselines: the actual Atlas revision queried for this
        # row. In event-base mode these match the packet/base revision; in
        # receipt-source mode they can vary per receipt.
