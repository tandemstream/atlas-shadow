...
| `GITHUB_ATLAS_SHADOW_TOKEN` | GitHub PAT or fine-grained token with `checks:write` + `pull_requests:write` on the target repo. Used to create check_runs and post PR comments. | Yes |
...
1. Generate a fine-grained PAT (recommended) or classic PAT scoped to the
   target repository. Required scopes: **Pull requests: read/write**,
   **Checks: read/write**, **Contents: read**.
...
| GH check stays in `in_progress` forever | Grader crashed before reaching `update_check_complete` | Check `.ingest-daemon.log` for traceback; `run_pr_grading` should update to `neutral` on errors, but a daemon process crash bypasses that |
